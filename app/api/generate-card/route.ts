...
const buildUserContext = (profile: {
  ...
  if (!profile) return "";
  const parts: string[] = [];
  if (profile.decisionPriority === "save_money") parts.push("コストを重視する傾向");
  if (profile.decisionPriority === "save_time") parts.push("時間効率を重視する傾向");
  if (profile.decisionPriority === "avoid_regret") parts.push("後悔を避けたい傾向");
  if (profile.decisionPriority === "discover_new") parts.push("新しいものを試したい傾向");
  if (profile.budgetSensitivity === "high") parts.push("予算に慎重");
  if (profile.dailyAvailableTime === "under_30m") parts.push("使える時間が少ない");
  if (parts.length === 0) return "";
  return `\n\nユーザーの傾向: ${parts.join("、")}。これを考慮して判断してください。`;
};
