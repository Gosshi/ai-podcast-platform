...
const normalizeStepError = (error: unknown): {
  ...
  return {
    status: 500,
    errorType: "tts_en_failed",
    message,
    jobRunError: safeJsonStringify({ ok: false, errorType: "tts_en_failed", message })
  };
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (failureCount >= MAX_FAILED_TTS_ATTEMPTS) {
      ...
      const message = `tts-en aborted: ${failureCount} failed attempts for this episode/audioVersion`;
