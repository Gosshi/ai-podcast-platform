...
const SYSTEM_PROMPT =
  ...
  "DeepDiveごとに判断フレーム（Frame A/B/C/D）を明示し、数値計算または条件判定で結論を出してください。" +
  "判断は個人視点に限定し、B2B視点は扱わないでください。" +
...
const toUserPrompt = (script: string): string => {
  return [
    ...
    "制約:",
    "- OPには『この番組はあなたの時間とお金を守る』『解説ではなく意思決定支援』を含める",
    "- DEEPDIVEの⑤は『今日の判断（個人視点）』で、Frame A/B/C/Dのいずれかを明示し、数値計算または条件判定と結論を必ず書いたうえで文末を『あなたはどうするか。』にする",
    "- DEEPDIVEの⑥は『判断期限（個人の行動期限）』、⑦は『監視ポイント（個人が見るべき数値）』にする",
    "- 判断は個人の時間とお金に限定する",
    "- 事業者予算配分、媒体配分、業界戦略、媒体再設計は書かない",
    "- 全DEEPDIVEで同じテンプレ判断（例: 30分固定）を繰り返さない",
    "- QuickNewsの判断タグは【今使う】【今使わない】【監視】のみ使う",
    "- QuickNewsでも可能な限りFrame A/B/C/Dを使って判断理由を示す",
