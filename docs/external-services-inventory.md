...
| サービス | 状態 | 用途 |
| --- | --- | --- |
| X API (旧 Twitter API) | アカウント作成済み / 手動運用開始可 / env 有効化待ち | エピソード公開後の自動投稿 |
| Apple Podcasts | 公開済み | Podcast RSS 配信先 |
...
- 注意点:
  ...
  - `Scheduled Daily Publish` は repo variable `ENABLE_SCHEDULED_DAILY_PUBLISH=true` の時だけ自動実行する
  - X 自動投稿 workflow は repo variable `ENABLE_X_AUTO_POST_WORKFLOW=true` かつ `X_AUTO_POST_ENABLED=true` と X credentials が揃っている時のみ自動実行する
  - manual dispatch は repo variable 未設定でも使える
...
- 状態:
  - `2026-03-23` 時点で公開済み
  - 公開 URL: `https://podcasts.apple.com/jp/podcast/%E5%88%A4%E6%96%AD%E3%81%AE%E3%81%98%E3%81%8B%E3%82%93-by-signalmove/id1887020163`
- 注意点:
...
- X 自動投稿を使うなら repo variable `ENABLE_X_AUTO_POST_WORKFLOW=true` を入れたうえで GitHub workflow の manual dispatch を 1 回確認する
