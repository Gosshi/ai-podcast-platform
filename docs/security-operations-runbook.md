...
- [x] `ADMIN_IP_ALLOWLIST` または `ADMIN_BASIC_AUTH_*` を本番に設定した
- [ ] 管理者ワンタイムコードの送達を確認した
- [x] ログイン通知メールの送達を確認した
- [x] アカウント変更通知メールの送達を確認した
- [ ] 開発端末の OS 更新と標準マルウェア保護が有効

## 確認メモ

- 2026-03-21: `ADMIN_BASIC_AUTH_*` を本番環境に設定し、`https://signal-move.com/admin/trends` で Basic 認証を通過した後に通常ログイン画面へ遷移することを確認。
- 2026-03-21: 本番 `RESEND_API_KEY` / `EMAIL_FROM` 設定後、ログイン通知メールの送達を確認。
- 2026-03-21: 通知設定を変更し、`ACCOUNT UPDATE` メールとして設定変更通知の送達を確認。
- 2026-03-21: `supabase db push` 実行後、`account_security_notification_state` テーブルを本番に反映。
