...
- 管理画面は一般会員画面と分離し、管理者メールの許可リストに加えて、管理者メール宛てのワンタイム確認コードを要求する。
...
- 必要に応じて `/admin/*` と `/api/admin/*` に対し、IP allowlist または HTTP Basic 認証の外側制限を追加で有効化できる。
- 管理画面へのアクセスには、通常の会員ログインに加えて、管理者メール宛ての 6 桁ワンタイム確認コードを要求する。
- 管理者向け追加アクセス確認では、確認コードをサーバーサイドでハッシュ化して保管し、10 回連続失敗で一定時間ロックする。
- 管理者向け追加アクセス確認 API に CSRF 検証とレート制限を適用する。

運用前提:
- `ADMIN_EMAILS` を本番環境に設定する。
- `ADMIN_ACCESS_SECRET`、`RESEND_API_KEY`、`EMAIL_FROM` を本番環境に設定する。
- `ADMIN_IP_ALLOWLIST` または `ADMIN_BASIC_AUTH_USER` / `ADMIN_BASIC_AUTH_PASSWORD` のいずれかを本番環境に設定し、管理画面の外側制御を有効にする。

運用確認:
- 2026-03-21 時点で本番環境に `ADMIN_BASIC_AUTH_*` を設定し、`/admin/*` で外側制限が有効であることを確認済み。

関連コード:
- [app/lib/adminGuard.ts](/Users/gota/Documents/src/ai-podcast-platform/app/lib/adminGuard.ts)
- [app/lib/adminPerimeter.ts](/Users/gota/Documents/src/ai-podcast-platform/app/lib/adminPerimeter.ts)
...
- [proxy.ts](/Users/gota/Documents/src/ai-podcast-platform/proxy.ts)
...
- [supabase/migrations/20260321170000_add_admin_access_challenges.sql](/Users/gota/Documents/src/ai-podcast-platform/supabase/migrations/20260321170000_add_admin_access_challenges.sql)

備考:
- Stripe 画面上の「二段階認証または二要素認証」は、会員ログイン済み状態に加えた管理者メール宛てのワンタイム確認コードで担保する運用を前提とする。
- 外側制限は `ADMIN_IP_ALLOWLIST` と `ADMIN_BASIC_AUTH_*` の設定で有効化できる。必要に応じて Cloudflare Access などの上位制御を併用する。
...
- 管理者向け再実行 API と管理者アクセス確認 API にも CSRF 検証とレート制限を適用している。
...
- セキュリティヘッダ (`CSP`, `HSTS`, `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`) を全ページに付与する。
- 初回の監査結果と運用手順は別紙の記録に残す。
...
- [app/api/stripe/subscription-checkout/route.ts](/Users/gota/Documents/src/ai-podcast-platform/app/api/stripe/subscription-checkout/route.ts)
- [app/api/stripe/billing-portal/route.ts](/Users/gota/Documents/src/ai-podcast-platform/app/api/stripe/billing-portal/route.ts)
- [app/api/admin/retry-daily-generate/route.ts](/Users/gota/Documents/src/ai-podcast-platform/app/api/admin/retry-daily-generate/route.ts)
- [app/api/admin/access/route.ts](/Users/gota/Documents/src/ai-podcast-platform/app/api/admin/access/route.ts)
...
- [docs/security-scan-evidence-2026-03-21.md](/Users/gota/Documents/src/ai-podcast-platform/docs/security-scan-evidence-2026-03-21.md)
...
- 運用手順として、端末保護・アップデート適用・週次セキュリティ確認を runbook に明記している。
...
運用確認:
- 2026-03-21 時点の Stripe Radar で、`risk_level = highest` の支払いブロックと Stripe blocklist 一致時のブロックが有効であることを確認済み。
...
運用確認:
- 2026-03-21 時点で本番 `RESEND_API_KEY` / `EMAIL_FROM` を設定し、ログイン通知メールの送達を確認済み。
- 2026-03-21 時点で通知設定変更後の `ACCOUNT UPDATE` メール送達を確認済み。
- 通知重複抑制用テーブル `account_security_notification_state` は本番 Supabase に反映済み。
